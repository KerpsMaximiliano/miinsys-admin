...
import { FirmaDatoAdicional, OpcionCuestionario, PreguntaCuestionario, ValidacionCuestionario } from 'src/app/models/cuestionario';
...
export class AbmCuestionarioModalComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: {mode: string, data?:any, ordenPregunta: number, idCuestionario: number, idSeccion: number, titulosSeccion: Array<string>, preguntaTipo?: string, empresaId: number},
  ) {
  ...
  ngOnInit(): void {
    ...
    if(this.data.mode == "Edit" || this.data.mode == "Copy") {
      ...
      switch (this.tipoElegido) {
        case 4:
          this.preguntaForm.get('textoFijo')?.setValue(this.data.data.opciones[0].opc_descripcion);
          if(this.data.data.opciones[0].opc_descripcion == undefined) {
            this.preguntaForm.get('textoFijo')?.setValue(this.data.data.opciones[0].descripcion);
          }
          break;
        case 5:
          if (this.data.data.validators.length > 0) {
            ...
            this.data.data.validators.forEach((val: { id_validacion: number; Value: string; valor: string}) => {
              if (val.id_validacion == 4) {
                if(val.valor != undefined) {
                  this.preguntaForm.get('maxLength')?.setValue(Number(val.valor));
                } else {
                  this.preguntaForm.get('maxLength')?.setValue(Number(val.Value));
                }
              };
              if (val.id_validacion == 3) {
                if(val.valor != undefined) {
                  this.preguntaForm.get('minLength')?.setValue(Number(val.valor));
                } else {
                  this.preguntaForm.get('minLength')?.setValue(Number(val.Value));
                }
              };
            });
          } else {
        ...
        case 6:
          ...
          this.data.data.opciones.forEach((opc: { descripcion: any, opc_descripcion: any, valor: any }) => {
            this.tpr6.push(opc);
          });
        ...
        case 10:
          console.log(this.data.data.opciones);
          this.data.data.opciones.forEach((opc: { descripcion: any; valor: any; }) => {
            this.tpr10.push(opc);
          });
        ...
        case 15:
          if(this.data.data.opciones[0].descripcion == undefined) {
            this.preguntaForm.get('opcion1')?.setValue(this.data.data.opciones[0].opc_descripcion);
          } else {
            this.preguntaForm.get('opcion1')?.setValue(this.data.data.opciones[0].descripcion);
          };
          if(this.data.data.opciones[1].descripcion == undefined) {
            this.preguntaForm.get('opcion2')?.setValue(this.data.data.opciones[1].opc_descripcion);
          } else {
            this.preguntaForm.get('opcion2')?.setValue(this.data.data.opciones[1].descripcion);
          };
          this.preguntaForm.get('critica')?.setValue(this.data.data.critica);
          if (this.data.data.validators.length > 0) {
            if(this.data.data.validators[0].id_validacion == 0) {
          ...
          break;
        case 16:
          this.cuestionarioService.getFirmaDatoAdicional(this.data.data.pre_id).subscribe(resFirma => {
  ...
  changeType() {
    ...
    switch (this.tipoElegido) {
      case 4:
        //Texto fijo
        this.preguntaForm.addControl('textoFijo', new FormControl(null, Validators.required));
        this.descripcion = "Campo de texto fijo que mostrará los datos ingresados";
        break;
      case 5:
        //Input Texto
        this.preguntaForm.addControl('requerido', new FormControl(true, Validators.required));
        this.preguntaForm.addControl('maxLength', new FormControl(null));
        this.preguntaForm.addControl('minLength', new FormControl(null));
        this.descripcion = "Campo para ingresar texto, con validaciones opcionales de caracteres mínimos y máximos";
        break;
      case 6:
        //Combo selección
        this.preguntaForm.addControl('requerido', new FormControl(true, Validators.required));
        ...
        this.descripcion = "Desplegable con las opciones agregadas en la grilla";
        break;
      case 7:
        //Input numérico
        this.preguntaForm.addControl('requerido', new FormControl(true, Validators.required));
        ...
        this.descripcion = "Campo para ingresar un número, con validaciones opcionales de número mínimo y número máximo";
        break;
      case 8:
        //Nombre usuario
        this.descripcion = "Campo inhabilitado que se autocompletará con el nombre del usuario realizando el cuestionario";
        break;
      case 9:
        //Fecha
        this.descripcion = "Campo inhabilitado que se autocompletará con la fecha en la cuál se inicie el cuestionario";
        break;
      case 10:
        //Selección radio
        this.preguntaForm.addControl('critica', new FormControl(false, Validators.required));
        this.preguntaForm.addControl('requerido', new FormControl(true, Validators.required));
        this.preguntaForm.addControl('opcion', new FormControl(null));
        this.preguntaForm.addControl('valor', new FormControl(null));
        this.descripcion = "Selección de una opción con nombres y valores determinados en la grilla. Las opciones que posean crítico 0 van a requerir justificación con registro fotográfico al ser seleccionadas";
        break;
      case 11:
        //RUT Usuario
        this.descripcion = "Campo inhabilitado que se autocompletará con el RUT del usuario realizando el cuestionario";
        break;
      case 12:
      ...
      case 13:
        //Autocompletar
        this.listaDatosService.getListaByParams({empresa: this.data.empresaId}).subscribe(d => {
          console.log(d)
          this.listasDeDatos = d.filter((x: { id_lista_datos: number; estado: number; }) => x.id_lista_datos == this.idListaExistente || x.estado == 1);
        })
        this.preguntaForm.addControl('requerido', new FormControl(true, Validators.required));
        this.preguntaForm.addControl('lista', new FormControl(null, Validators.required));
        this.descripcion = "Lista de datos a vincular. La primera columna de la lista asociada será el valor a seleccionar, mientras que los demás datos de la fila serán autocompletados";
        break;
      case 14:
        //Subir foto
        this.preguntaForm.addControl('requerido', new FormControl(true, Validators.required));
      ...
      case 15:
        ...
        this.preguntaForm.addControl('critica', new FormControl(false, Validators.required));
        this.preguntaForm.addControl('requerido', new FormControl(true, Validators.required));
        this.preguntaForm.addControl('opcion1', new FormControl("SI", Validators.required));
        this.preguntaForm.addControl('opcion2', new FormControl("NO", Validators.required));
        this.descripcion = "Selección de una opción de dos posibles opciones. La opción 2 requerirá justificación y registro fotográfico al ser seleccionada";
        break;
      case 16:
        this.preguntaForm.addControl('requerido', new FormControl(true, Validators.required));
      ...
      case 17:
        ...
        this.preguntaForm.addControl('maxLength', new FormControl(null));
        this.preguntaForm.addControl('minLength', new FormControl(null));
        this.descripcion = "Campo para ingresar texto alfanumérico, con validaciones opcionales de caracteres mínimos y máximos";
        break;
    }
  }

  save() {
    let find = this.data.titulosSeccion.find(t => t.toLowerCase() == this.tituloPregunta.toLowerCase());
    if (this.data.mode == "Edit" && this.tituloPregunta.toLowerCase() != this.data.data.pre_descripcion.toLowerCase() && find != undefined) {
      this.openSnackBar('El título ya existe en esta sección', 'X', 'warn-snackbar');
      return;
    ...
    switch (this.tipoElegido) {
      ...
      case 5:
        ...
        if(this.preguntaForm.get('maxLength')?.value != null && this.preguntaForm.get('maxLength')?.value != '') {
          model.preguntaValidaciones!.push({
            id_pregunta_validacion: 0,
            id_validacion: 4,
            valor: `${this.preguntaForm.get('maxLength')?.value}`,
            mensaje: `Máximo ${this.preguntaForm.get('maxLength')?.value} caracteres`,
            id_estado: 1
          })
        }
        if(this.preguntaForm.get('minLength')?.value != null && this.preguntaForm.get('minLength')?.value != '') {
          model.preguntaValidaciones!.push({
            id_pregunta_validacion: 0,
            id_validacion: 3,
            valor: `${this.preguntaForm.get('minLength')?.value}`,
            mensaje: `Mínimo ${this.preguntaForm.get('minLength')?.value} caracteres`,
            id_estado: 1
          })
        }
        break;
      ...
      case 7:
        ...
        if(this.preguntaForm.get('min')?.value != null && this.preguntaForm.get('min')?.value != '') {
          model.preguntaValidaciones!.push({
            id_pregunta_validacion: 0,
            id_validacion: 2,
        ...
        if(this.preguntaForm.get('max')?.value != null && this.preguntaForm.get('max')?.value != '') {
          model.preguntaValidaciones!.push({
            id_pregunta_validacion: 0,
            id_validacion: 1,
      ...
      case 12:
        ...
        break;
      case 13:
        Object.assign(model, {id_lista: this.preguntaForm.get('lista')?.value});
        model.opciones = [{
          id_opcion: 0,
        ...
        for (let index = 0; index < 2; index++) {
          if(this.data.data) {
            if (this.data.data.opciones[index].opc_id != undefined) {
              model.opciones[index].id_opcion = this.data.data.opciones[index].opc_id;
            } else if(this.data.data.opciones[index].id_opcion != undefined) {
              model.opciones[index].id_opcion = this.data.data.opciones[index].id_opcion;
            }
          }
        }
        break;
    ...
    if(this.data.mode == 'Edit') {
      this.cuestionarioService.updatePregunta(model, this.data.idCuestionario, this.data.idSeccion, idPregunta).subscribe(d => {
        if (model.id_tipo_pregunta == 16 && this.tpr16.length > 0) {
          ...
          this.cuestionarioService.updateFirmaDatoAdicional({ firmaDatoAdicional: arrayFirmas }).subscribe(res => {
      ...
      console.log(model)
      this.cuestionarioService.addPregunta(model, this.data.idCuestionario, this.data.idSeccion).subscribe(d => {
        ...
        if (model.id_tipo_pregunta == 16 && this.tpr16.length > 0) {
          ...
          this.cuestionarioService.createFirmaDatoAdicional({ firmaDatoAdicional: arrayFirmas }).subscribe(res => {
            this.dialogRef.close(model);
            //window.location.reload();
            let url = this.router.url;
            this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
  ...
  checkSave():boolean {
    ...
    if(this.tipoElegido == 6 && (this.tpr6.length == 0 || this.tpr6.find(data => data.opc_descripcion == '') != undefined)) {
      return true;
    };
    if(this.tipoElegido == 10 && (this.tpr10.length == 0 || this.tpr10.find(data => data.opc_descripcion == '') != undefined)) {
      return true;
    };
    return false;
  }
