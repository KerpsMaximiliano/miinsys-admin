...
export class RespuestasCuestionarioVisualizarComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.respuestaCuestionarioId =
      this.activatedRoute.snapshot.params['respuestaCuestionarioId'];

    this.respuestasCuestionarioService
      .getRespuesta(this.respuestaCuestionarioId)
      .subscribe((res) => {
        ...
        this.datosUsuarioForm.patchValue({
          rut: res.rut,
          ubicacion: res.info_gps,
          fechaCarga: this.formatDate(res.fechaCarga),
        });
        this.datosUsuarioForm.disable();

        this.respuestasCuestionarioService
          .getCuestionario(this.cuestionarioId)
          .subscribe((d) => {
            this.modeloCuestionario = d;

            this.dataSource = this.modeloCuestionario.secciones.find(
              (sec) => sec.orden == 0
            )!.preguntas;

            this.tableDataChange();
            this.cabeceraSecId = this.modeloCuestionario.secciones.find(
              (sec) => sec.orden == 0
            )!.sec_id;
            this.modeloCuestionario.secciones.sort(function (a, b) {
  ...
  openLista(row: any) {
    const dialogRef = this.dialog.open(RespuestasCuestionariosModalComponent, {
      ...
      data: {
        id: row.imgId,
        type: 'list',
        preguntaId: row.preId,
        respuestaCuestionarioId: this.respuestaCuestionarioId,
      },
    });
  ...
  tableDataChange() {
    this.modeloCuestionario.secciones.forEach((sec) => {
      ...
      sec.preguntas.forEach((pre) => {
        if (
          this.modeloRespuestas.secciones.find(
            (s) => s.seccionId == modelSeccion.seccionId
          ) != undefined
        ) {
          if (
            this.modeloRespuestas.secciones
              .find((s) => s.seccionId == modelSeccion.seccionId)!
              .preguntas.find((p) => p.preguntaId == pre.pre_id) != undefined
          ) {
            let modelPregunta: TablaRespuesta = {
              tipo: pre.tpr_id,
              descripcion: pre.pre_descripcion,
              preId: pre.pre_id,
              valor: this.modeloRespuestas.secciones
                .find((s) => s.seccionId == modelSeccion.seccionId)!
                .preguntas.find((p) => p.preguntaId == pre.pre_id)!
                .preguntaValor,
              imgId: this.modeloRespuestas.secciones
                .find((s) => s.seccionId == modelSeccion.seccionId)!
                .preguntas.find((p) => p.preguntaId == pre.pre_id)!
                .preguntaImagenId,
            };
            if (pre.tpr_id == 10 || pre.tpr_id == 15) {
              modelPregunta.valor = pre.opciones.find(
                (o) => o.opc_valor == Number(modelPregunta.valor)
              )!.opc_descripcion;
            }
    ...
    this.changeSeccion(
      this.tablaRespuestas.find((res) => res.orden == 0)!.seccionId
    );
  }
}
